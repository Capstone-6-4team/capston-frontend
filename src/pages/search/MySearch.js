...
const MySearch = () => {
    ...
    const onSubmit = () => {
        if (mainAddress && subAddress) {
            const dest = "/search?city=" + mainAddress + "&district=" + subAddress;
            window.location = dest;
        }
    }

    useEffect(() => {
        setMainAddress("서울특별시");
    }, []);
            ...
            <div class="w-1/2 flex mx-auto h-12 ">
                <div class="flex w-full ">
                    ...
                    <div class="w-1/2 p-2 mx-2 bg-gray-200 rounded">
                        <Select options={Object.keys(AdministrativeDistrict)} name="mainAddress" onChange={onMainAddressChange} />
                    </div>
                    <div class="w-1/2 p-2 mx-2 bg-gray-200 rounded">
                        <Select options={AdministrativeDistrict[mainAddress]} name="subAddress" onChange={onSubAddressChange} />
                    </div>
                </div>
                <div class="w-24 h-full bg-representative-color rounded">
                    <button class="w-full h-full" onClick={onSubmit}>
                        <span class="m-auto text-white">검색</span>
                    </button>
                </div>
            </div>
